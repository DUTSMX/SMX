...
router.post('/search', function (req, res) {
    ...
    if (type == "课程") {
        console.log("type:课程")
        course.search(word, function (courseResult) {
            var data = {course: courseResult, video: [], question: []}
            console.log("data:" + JSON.stringify(data));
            res.send(data)
        })
    } else if (type == "视频") {
        console.log("type:视频")
        video.searchVideo(word, function (videoResult) {
            var data = {course: [], video: videoResult, question: []}
            console.log("data:" + JSON.stringify(data));
            res.send(data)
        })
    } else if (type == "问答") {
        console.log("type:问答")
        question.searchQuestion(word, function (questionResult) {
            var data = {course: [], video: [], question: questionResult}
            console.log("data:" + JSON.stringify(data));
            res.send(data)
        })
    } else {
        ...
        course.search(word, function (courseResult) {
            video.searchVideo(word, function (videoResult) {
                question.searchQuestion(word, function (questionResult) {
                    var data = {course: courseResult, video: videoResult, question: questionResult}
                    console.log("data:" + JSON.stringify(data));
                    res.send(data)
                })
            })
